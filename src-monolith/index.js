...
import { Group,Euler,Matrix4,Vector3, AxesHelper, GridHelper, MeshPhysicalNodeMaterial, Matrix3 } from "three";
import { min, smoothstep,select,If, Fn, step,vec3, vec2, float, max, transformNormalToView, positionGeometry,normalGeometry, uniform, mix } from 'three/tsl';
...
var tslPositionNode = Fn( ( )=>{

console.timeLog('TSL','position');

	var pos = positionGeometry;
		
	var p = pos.toVar(); // output position
...
var tslNormalNode = Fn( ( )=>{

console.timeLog('TSL','normal');
	
	var pos = positionGeometry;
		
	var p = normalGeometry.toVar(); // output position
	...
	If( pos.x.greaterThan(0), ()=>{
		...
		var isLFoot = tslIsFoot( pos ).toVar();
		If( isLFoot.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_FOOT_MAT), isLFoot ) );
		});
		
		// left foot
		var isLAnkle = tslIsAnkle( pos ).toVar();
		If( isLAnkle.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_ANKLE_MAT), isLAnkle ) );
		});

		// left knee
		var isLKnee = tslIsKnee( pos ).toVar();
		If( isLKnee.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_KNEE_MAT), isLKnee ) );
		});

		// left leg
		var isLLeg = tslIsLeg( pos, 1 ).toVar();
		If( isLLeg.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_LEG_MAT), isLLeg ) );
		});
		...
		var isLWrist = tslIsWrist( pos, 1 ).toVar();
		If( isLWrist.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_WRIST_MAT), isLWrist ) );
		});

		// left forearm
		var isLForearm = tslIsForearm( pos, 1 ).toVar();
		If( isLForearm.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_FOREARM_MAT), isLForearm ) );
		});

		// left elbow
		var isLElbow = tslIsElbow( pos, 1 ).toVar();
		If( isLElbow.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_ELBOW_MAT), isLElbow ) );
		});

		// left arm
		var isLArm = tslIsArm( pos, 1 ).toVar();
		If( isLArm.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(L_ARM_MAT), isLArm ) );
		});
	...
	If( pos.x.lessThan(0), ()=>{
		...
		var isRFoot = tslIsFoot( pos ).toVar();
		If( isRFoot.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_FOOT_MAT), isRFoot ) );
		});

		// right foot
		var isRAnkle = tslIsAnkle( pos ).toVar();
		If( isRAnkle.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_ANKLE_MAT), isRAnkle ) );
		});

		// right knee
		var isRKnee = tslIsKnee( pos ).toVar();
		If( isRKnee.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_KNEE_MAT), isRKnee ) );
		});

		// right leg
		var isRLeg = tslIsLeg( pos, -1 ).toVar();
		If( isRLeg.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_LEG_MAT), isRLeg ) );
		});
		...
		var isRWrist = tslIsWrist( pos, -1 ).toVar();
		If( isRWrist.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_WRIST_MAT), isRWrist ) );
		});

		// right forearm
		var isRForearm = tslIsForearm( pos, -1 ).toVar();
		If( isRForearm.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_FOREARM_MAT), isRForearm ) );
		});

		// right elbow
		var isRElbow = tslIsElbow( pos, -1 ).toVar();
		If( isRElbow.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_ELBOW_MAT), isRElbow ) );
		});

		// right arm
		var isRArm = tslIsArm( pos, -1 ).toVar();
		If( isRArm.greaterThan(0), ()=>{
			p.assign( mix( p, p.mul(R_ARM_MAT), isRArm ) );
		});
	...
	var isHead = tslIsHead(pos).toVar();
	If( isHead.greaterThan(0), ()=>{
		p.assign( mix( p, p.mul(HEAD_MAT), isHead ) );
	});
		
	// neck
	var isNeck = tslIsNeck( pos ).toVar();
	If( isNeck.greaterThan(0), ()=>{
		p.assign( mix( p, p.mul(NECK_MAT), isNeck ) );
	});
	
	// chest
	var isChest = tslIsChest(pos).toVar();
	If( isChest.greaterThan(0), ()=>{
		p.assign( mix( p, p.mul(CHEST_MAT), isChest ) );
	});
	...
	var isWaist = tslIsWaist(pos).toVar();
	If( isWaist.greaterThan(0), ()=>{
		p.assign( mix( p, p.mul(WAIST_MAT), isWaist ) );
	});

	// torso
	p.assign( p.mul(TORSO_MAT) );

	return transformNormalToView( p );
...
model.material.positionNode = tslPositionNode();
model.material.normalNode = tslNormalNode();
