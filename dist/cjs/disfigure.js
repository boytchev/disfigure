...
function selectHipLeft( { hipLeftSpan } ) {
	...
	return y
		.smoothstep( hipLeftSpan.x, hipLeftSpan.y )
...
function selectHipRight( { hipRightSpan } ) {
	...
	return y
		.smoothstep( hipRightSpan.x, hipRightSpan.y )
...
var tslEmissiveNode = tsl.Fn( ( posture )=>{
	...
	var k = tsl.float( 0 )
		...
		.add( selectHipRight( posture ).mul( tsl.select( s.equal( 11 ), 1, 0 ) ) )
		.add( selectLegRight( posture ).mul( tsl.select( s.equal( 12 ), 1, 0 ) ) )
		.add( selectKneeRight( posture ).mul( tsl.select( s.equal( 13 ), 1, 0 ) ) )
		.add( selectAnkleRight( posture ).mul( tsl.select( s.equal( 14 ), 1, 0 ) ) )
		...
		.add( selectArmRight( posture ).mul( tsl.select( s.equal( 21 ), 1, 0 ) ) )
		.add( selectElbowRight( posture ).mul( tsl.select( s.equal( 22 ), 1, 0 ) ) )
		.add( selectForearmRight( posture ).mul( tsl.select( s.equal( 23 ), 1, 0 ) ) )
		.add( selectWristRight( posture ).mul( tsl.select( s.equal( 24 ), 1, 0 ) ) )
		.toVar( );
